...
/* ============================================================
   CONFIGURATION — à ajuster selon ta base Firestore existante
   ============================================================ */
const CONFIG = {
  // Nom de la collection Firestore contenant les cours
  coursesCollection: 'courses',
  // Nom de la collection où seront enregistrées les réservations d'essai
  bookingsCollection: 'essai_reservations',
  // Collection utilisée par l'extension Firebase "Trigger Email"
  mailCollection: 'mail',
  // Adresse recevant le récapitulatif de chaque réservation
  adminEmail: 'contact@mntstudiodance.fr', // ⚠️ à remplacer par la vraie adresse
  // Nombre de prochaines occurrences proposées par cours
  weeksAhead: 2,
  // Champs attendus dans un document "course". Adapte les clés à droite
  // si tes noms de champs Firestore diffèrent.
  fields: {
    name: 'nom',
    day: 'jour', // ex: "Lundi" ou 1 (0=dimanche...6=samedi)
    site: 'site',
    teacher: 'professeur',
    time: 'horaire',
    level: 'niveau',
...
const SITES_COLORS = {
  default: 'from-magenta-deep to-magenta',
}
...
function nextOccurrences(dayIndex, count) {
  const results = []
  if (dayIndex === null) return results
  const today = new Date()
  today.setHours(0, 0, 0, 0)
  let cursor = new Date(today)
  // avance jusqu'au premier jour correspondant (aujourd'hui inclus)
...
export default function App() {
  const [step, setStep] = useState(0) // 0 hero, 1 courses, 2 dates, 3 infos, 4 recap, 5 success
  const [courses, setCourses] = useState([])
  const [loading, setLoading] = useState(true)
  const [loadError, setLoadError] = useState(null)
  const [selectedCourses, setSelectedCourses] = useState([])
  const [selectedDates, setSelectedDates] = useState({}) // courseId -> Date
  ...
  useEffect(() => {
    async function loadCourses() {
      try {
        const snap = await getDocs(collection(db, CONFIG.coursesCollection))
        const list = snap.docs.map((d) => {
          const data = d.data()
          const f = CONFIG.fields
          return {
            id: d.id,
            name: data[f.name] || 'Cours',
            dayIndex: normalizeDayToIndex(data[f.day]),
            dayLabel:
              typeof data[f.day] === 'string'
                ? data[f.day]
                : FRENCH_DAYS[normalizeDayToIndex(data[f.day])] || '',
            site: data[f.site] || '',
            teacher: Array.isArray(data[f.teacher])
              ? data[f.teacher].join(' & ')
              : data[f.teacher] || '',
            time: data[f.time] || '',
            level: data[f.level] || '',
        ...
        console.error(err)
        setLoadError(
          "Impossible de charger les cours pour le moment. Vérifie la configuration Firebase (src/firebase.js) et le nom de la collection."
        )
      } finally {
        setLoading(false)
      }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      // Mail récapitulatif pour le studio
...
function Header({ step }) {
  const labels = ['', 'Cours', 'Date', 'Coordonnées', 'Récapitulatif', 'Confirmé']
  return (
